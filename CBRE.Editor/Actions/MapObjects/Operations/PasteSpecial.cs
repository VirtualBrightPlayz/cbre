...
namespace CBRE.Editor.Actions.MapObjects.Operations {
    public class PasteSpecial : CreateEditDelete {
        ...
        public PasteSpecial(IEnumerable<MapObject> objectsToPaste, int numCopies,
            ...
            Vector3 offset, Vector3 rotation, bool makeEntitesUnique,
        ...
        private Vector3 GetPasteOrigin(Document document) {
        ...
        private IEnumerable<MapObject> CreateCopy(IDGenerator gen, Vector3 origin, Vector3 rotation, List<string> names, TransformFlags transformFlags) {
            var box = new Box(_objectsToPaste.Select(x => x.BoundingBox));
