...
VertexShaderOutput VertexShaderF(VertexShaderInput input)
{
    ...
    output.SelectColor = input.SelectColor;
...
float4 PixelShaderF(VertexShaderOutput input) : COLOR0
{
    ...
    float4 c = tex2D(TextureSampler, input.TexCoord) * float4(lighting, lighting, lighting, 1.0) * input.SelectColor;
